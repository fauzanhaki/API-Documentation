...
module.exports = {
    ...
    try {
      ...
      const transaction = await prisma.$transaction(async (prisma) => {
        const updatedSourceBalance =
          BigInt(sourceAccount.balance) - BigInt(amount);
        ...
        const updatedDestinationBalance =
          BigInt(destinationAccount.balance) + BigInt(amount);
        await prisma.bank_accounts.update({
          where: { id: destination_account_id },
          data: {
  ...
  getTransactions: async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Error fetching all transactions!" });
    }
  },
  ...
  getTransactionById: async (req, res) => {
    try {
      ...
      transaction.source_account.balance = parseInt(
        transaction.source_account.balance
      );
      transaction.destination_account.balance = parseInt(
  ...
  updateTransactionById: async (req, res) => {
    try {
      ...
      if (!existingTransaction) {
        return res
          .status(404)
          .json({ error: `Transaction with ID ${transactionId} not found!` });
      }
